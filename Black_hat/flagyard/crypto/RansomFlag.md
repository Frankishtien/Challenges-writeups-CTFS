...
# 1. فك تشفير الـ obfuscated_key من Layer 3 (RSA)
decoded_chars = [pow(c, d, n) for c in encrypted_message]
rsa_decoded_str = ''.join(chr(x) for x in decoded_chars)
obfuscated_key = base64.b64decode(rsa_decoded_str)

# 2. إزالة الـ obfuscation بالـ XOR مع "0x1337"
xor_key = "0x1337"
real_key = bytes([obfuscated_key[i] ^ ord(xor_key[i % len(xor_key)]) for i in range(len(obfuscated_key))])

# 3. فك تشفير Layer 1 (ChaCha20)
nonce = base64.b64decode(nonce_b64)
cipher = ChaCha20.new(key=real_key, nonce=nonce)
flag = cipher.decrypt(base64.b64decode(encrypted_flag_b64)).decode()
